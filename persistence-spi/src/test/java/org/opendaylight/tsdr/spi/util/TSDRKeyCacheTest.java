...
public class TSDRKeyCacheTest {
    ...
    private static final String TSDR_TEST_KEY2 =
            "[NID=openflow:11][DC=EXTERNAL][MN=Memory][RK=hello:world,Testing:test]";
    private static final String TSDR_TEST_KEY3 = "[NID=openflow:11][DC=EXTERNAL][MN=Memory][RK=he:world,Testing:test]";
    private static final String TSDR_TEST_KEY4 =
            "[NID=openflow:11][DC=EXTERNAL][MN=Memory][RK=hello:worl,Testing:test]";
    private static final String TSDR_TEST_5080_KEY1 = "[NID=openflow:2][DC=QUEUESTATS][MN=TransmittedPackets]["
        + "RK=Node:openflow:2,NodeConnector:openflow:2:2,Queue:1]";
    private static final String TSDR_TEST_5080_KEY2 = "[NID=openflow:2][DC=QUEUESTATS][MN=TransmittedPackets]["
        + "RK=Node:openflow:2,NodeConnector:openflow:2:1,Queue:1]";
    ...
    private static final String TSDR_TEST_5052_KEY1 =
            "[NID=openflow:1][DC=FLOWTABLESTATS][MN=PacketLookup][RK=Node:openflow:1,Table:150]";
    private static final String TSDR_TEST_5052_KEY2 =
            "[NID=openflow:1][DC=FLOWTABLESTATS][MN=PacketLookup][RK=Node:openflow:1,Table:160]";

    private TSDRKeyCache keyCache;
    ...
    private class TestMetricJob implements TSDRKeyCache.TSDRMetricCollectJob {
        @Override
        public void collectMetricRecords(TSDRCacheEntry entry, long startDateTime, long endDateTime, int recordLimit,
                List<TSDRMetricRecord> globalResult) {
            TSDRMetricRecordBuilder builder = new TSDRMetricRecordBuilder();
            builder.setNodeID(entry.getNodeID());
            builder.setTSDRDataCategory(entry.getDataCategory());
            builder.setMetricName(entry.getMetricName());
            builder.setRecordKeys(entry.getRecordKeys());
            builder.setTimeStamp(System.currentTimeMillis());
            globalResult.add(builder.build());
        }
    }

    private class TestLogJob implements TSDRKeyCache.TSDRLogCollectJob {
        @Override
        public void collectLogRecords(TSDRCacheEntry entry, long startDateTime, long endDateTime, int recordLimit,
    ...
    public void testMetricByNode() {
        keyCache.addTSDRCacheEntry(TSDR_TEST_KEY);
        List<TSDRMetricRecord> list = keyCache.getTSDRMetricRecords(
                FormatUtil.KEY_NODEID + FormatUtil.getNodeIdFromTSDRKey(TSDR_TEST_KEY) + "]", 0, Long.MAX_VALUE, 1000,
                new TestMetricJob());
    ...
    public void testMetricByDataCategory() {
        keyCache.addTSDRCacheEntry(TSDR_TEST_KEY);
        List<TSDRMetricRecord> list = keyCache.getTSDRMetricRecords(
                FormatUtil.KEY_CATEGORY + FormatUtil.getDataCategoryFromTSDRKey(TSDR_TEST_KEY) + "]", 0, Long.MAX_VALUE,
                1000, new TestMetricJob());
        Assert.assertEquals(1, list.size());
        TSDRMetricRecord entry = list.get(0);
        Assert.assertEquals("openflow:1", entry.getNodeID());
        Assert.assertEquals(DataCategory.EXTERNAL, entry.getTSDRDataCategory());
    ...
    public void testMetricByMetricName() {
        keyCache.addTSDRCacheEntry(TSDR_TEST_KEY);
        List<TSDRMetricRecord> list = keyCache.getTSDRMetricRecords(
                FormatUtil.KEY_METRICNAME + FormatUtil.getMetriNameFromTSDRKey(TSDR_TEST_KEY) + ']', 0, Long.MAX_VALUE,
                1000, new TestMetricJob());
        Assert.assertEquals(1, list.size());
        TSDRMetricRecord entry = list.get(0);
        Assert.assertEquals("openflow:1", entry.getNodeID());
    ...
    public void testMetricByRecordKey() {
        keyCache.addTSDRCacheEntry(TSDR_TEST_KEY);
        List<TSDRMetricRecord> list = keyCache.getTSDRMetricRecords(FormatUtil.KEY_RECORDKEYS + "hello:world" + ']', 0,
                Long.MAX_VALUE, 1000, new TestMetricJob());
        Assert.assertEquals(1, list.size());
        TSDRMetricRecord entry = list.get(0);
        Assert.assertEquals("openflow:1", entry.getNodeID());
        Assert.assertEquals(DataCategory.EXTERNAL, entry.getTSDRDataCategory());
    ...
    public void testLogByNode() {
        keyCache.addTSDRCacheEntry(TSDR_TEST_KEY);
        List<TSDRLogRecord> list = keyCache.getTSDRLogRecords(
                FormatUtil.KEY_NODEID + FormatUtil.getNodeIdFromTSDRKey(TSDR_TEST_KEY) + "]", 0, Long.MAX_VALUE, 1000,
                new TestLogJob());
        Assert.assertEquals(1, list.size());
    ...
    public void testLogByRecordKey() {
        keyCache.addTSDRCacheEntry(TSDR_TEST_KEY);
        List<TSDRLogRecord> list = keyCache.getTSDRLogRecords(FormatUtil.KEY_RECORDKEYS + "hello:world" + ']', 0,
                Long.MAX_VALUE, 1000, new TestLogJob());
        Assert.assertEquals(1, list.size());
        TSDRLogRecord entry = list.get(0);
        Assert.assertEquals("openflow:1", entry.getNodeID());
        Assert.assertEquals(DataCategory.EXTERNAL, entry.getTSDRDataCategory());
    ...
    public void tesBugFixPrefix() {
        ...
        List<TSDRLogRecord> list = keyCache.getTSDRLogRecords(FormatUtil.KEY_NODEID + "openflow:1]", 0, Long.MAX_VALUE,
                1000, new TestLogJob());
        Assert.assertEquals(1, list.size());
        TSDRLogRecord entry = list.get(0);
        Assert.assertEquals("openflow:1", entry.getNodeID());
    ...
    public void tesBugFixRKKey() {
        ...
        List<TSDRLogRecord> list = keyCache.getTSDRLogRecords(FormatUtil.KEY_RECORDKEYS + "he:world]", 0,
                Long.MAX_VALUE, 1000, new TestLogJob());
        Assert.assertEquals(1, list.size());
        TSDRLogRecord entry = list.get(0);
    ...
    public void tesBugFixRKValue() {
        ...
        keyCache.addTSDRCacheEntry(TSDR_TEST_KEY3);
        keyCache.addTSDRCacheEntry(TSDR_TEST_KEY4);
        List<TSDRLogRecord> list = keyCache.getTSDRLogRecords(FormatUtil.KEY_RECORDKEYS + "hello:worl]", 0,
                Long.MAX_VALUE, 1000, new TestLogJob());
        Assert.assertEquals(1, list.size());
    }
