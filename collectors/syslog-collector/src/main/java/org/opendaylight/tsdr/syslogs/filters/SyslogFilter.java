...
public interface SyslogFilter {
     ...
     */
    boolean match(String syslog);

    /**
     * if "match" then parse the syslog while filtering it.
     * If null is returned then the syslog is dropped.
     * Sometimes there is a syslog forwarder and this is its address. The originator address is in the syslog body,
     * so we might want to filter according to that.
     *
     ...
     */
    TSDRLogRecord filterAndParseSyslog(String syslog,String forwarderAddress,String originatorAddress);

    /**
     * Determinate the persistence destination of the syslog. Currently we have only one, we might have multiple in
     * the future.
     */
    int getPersistenceDestination(TSDRLogRecord logRecord);
}
