 ...
 **/
// This container aggregate several metrics builders that correlate to the same
// InstanceIdentifier.
// It has an array of builders that their metric value is update by notification
// the class assums that the order of initially adding the different metrics to
// the container
// is also kept during update so it the metrics were added as "A,B,C" to the
// container
// the place in the array will be 0,1,2 so when updating metric A you need to
// update builder[0] & so on...
