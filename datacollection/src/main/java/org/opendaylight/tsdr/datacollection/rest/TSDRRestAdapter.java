...
public class TSDRRestAdapter implements Runnable {
    ...
    public TSDRRestAdapter(TSDRDOMCollector _collector) {
    ...
    public void run() {
        while (this.collector.isRunning()) {
            try {
    ...
    public String executeRequest(ITSDRRequest request) {
        ...
        if (maxDataPoints == 0)
            return "[]";
        Future<RpcResult<GetMetricOutput>> metric = collector.getTSDRService()
